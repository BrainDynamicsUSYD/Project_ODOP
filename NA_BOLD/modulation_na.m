function [visibilityd,MaxValued] = modulation_na(matrix_didecr) 
...
for kkm=1:size(store_min1d,1)
    if length(store_min1d) == 1
        store_min1_min1d = store_min1d;
    elseif length(store_min1d) > indMaxValued && kkma(1) < kkmi(1)
        store_min1_min1d = store_min1d(indMaxValued);
    elseif length(store_min1d) > indMaxValued && kkma(1) > kkmi(1)
        store_min1_min1d = store_min1d(indMaxValued+1);
    elseif length(store_min1d) == indMaxValued
        store_min1_min1d = store_min1d(indMaxValued);
    else
        store_min1_min1d = 0;
    end
