...
params.Gamma = 1.6; % wave damping rate
params.v_b = 1e-3; % propagation speed
...
if orientation == 0 || orientation == 90
    ...
    [visibility_BOLD,Maximum] = modulation_diag(matrix_rowx);
...
if orientation == 45 || orientation == 135
    ...
    [visibilityd_BOLD,Maximumd] = modulation_diag(matrix_didecr);
...
if orientation == 0 || orientation == 90
    ...
    [visibility_Wiener,MaxWiener] = modulation_diag(Wiener_rowx);
    % Modulation Wiener 0.125 mm
    [visibility_Wiener_noise,MaxWiener_noise] = modulation_diag(Wiener_noise_rowx);
end
...
if orientation == 45 || orientation == 135
    ...
    % Modulation Wiener 0.125 mm
    [visibilityd_Wiener,~] = modulation_diag(Wiener_didecr);
    % Modulation noise Wiener 0.125 mm
    [visibilityd_Wiener_noise,~] = modulation_diag(Wiener_noise_didecr);
...
if orientation == 0 || orientation == 90
    ...
    % Modulation BOLD res
    [visibility_BOLD_res,MaxBOLD_res] = modulation_diag(BOLD_res_rowx);
    visibility_BOLD_res_all(jj) = visibility_BOLD_res;
    MaxBOLD_res_all(jj) = MaxBOLD_res;
    % Modulation BOLD res pixelated
    [visibility_BOLD_res_pi,~] = modulation_diag(BOLD_res_pi_rowx);
...
if orientation == 45 || orientation == 135
    ...
    % Modulation BOLD res
    [visibilityd_BOLD_res,MaxBOLD_resd] = modulation_diag(BOLD_res_didecr);
    visibilityd_BOLD_res_all(jj) = visibilityd_BOLD_res;
    % Modulation BOLD res pixelated
    [visibilityd_BOLD_res_pi,~] = modulation_diag(BOLD_res_pi_didecr);
...
if orientation == 0 || orientation == 90
    ...
    [visibility_Wiener_res,MaxWiener_res] = modulation_diag(Wiener_res_rowx);
    visibility_Wiener_res_all(jj) = visibility_Wiener_res;
    MaxWiener_res_all(jj) = MaxWiener_res;
    % Modulation Wiener res
    [visibility_Wiener_res_pi,~] = modulation_diag(Wiener_res_pi_rowx);
...
if orientation == 45 || orientation == 135
    ...
    % Modulation Wiener 0.125 mm
    [visibilityd_Wiener_res,MaxWiener_res] = modulation_diag(Wiener_res_didecr);
    visibilityd_Wiener_res_all(jj) = visibilityd_Wiener_res;
    % Modulation Wiener 0.125 mm pixelated
    [visibilityd_Wiener_res_pi,~] = modulation_diag(Wiener_res_pi_didecr);
...
if orientation == 0 || orientation == 90
    ...
    % Modulation Wiener res
    [visibility_Wiener_noise_res,MaxWiener_noise_res] = modulation_diag(Wiener_noise_res_rowx);
    visibility_Wiener_noise_res_all(jj) = visibility_Wiener_noise_res;
    MaxWiener_noise_res_all(jj) = MaxWiener_noise_res;
    % Modulation Wiener res
    [visibility_Wiener_noise_res_pi,~] = modulation_diag(Wiener_noise_res_pi_rowx);
...
if orientation == 45 || orientation == 135
    ...
    % Modulation Wiener 0.125 mm
    [visibilityd_Wiener_noise_res,MaxWiener_noise_res] = modulation_diag(Wiener_noise_res_didecr);
    visibilityd_Wiener_noise_res_all(jj) = visibilityd_Wiener_noise_res;
    % Modulation Wiener 0.125 mm
    [visibilityd_Wiener_noise_res_pi,~] = modulation_diag(Wiener_noise_res_pi_didecr);
